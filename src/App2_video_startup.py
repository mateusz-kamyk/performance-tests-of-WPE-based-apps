...
# 2. Click OK to start playing video
# 3. Start measuring time
# 4. Detect the video playing tamplate
# 5. Stop measuring time
# 6. Add the result to the list

T9T = []

def App2_video_startup():
        ...
        for r in range(1,11):
            ...
            temp_w, temp_h = Template_list.App2_video_playback_template.shape[::-1]
            ...
            while True:
                ...
                result = cv2.matchTemplate(gray, Template_list.App2_video_playback_template, cv2.TM_CCOEFF_NORMED)
                ...
                if max_val > 0.95:
                    ...
                    print('T9T', '{}'.format(end_time - start_time))
                    T9T.append((utils.no, (end_time - start_time).total_seconds()))
                    n=1
                    break
                elif datetime.now() > loop_time_end:
                    end_time = datetime.now()
                    print('T9T', 'Time exceeded - unsuccessful test')
